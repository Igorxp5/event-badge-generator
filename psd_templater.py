...
@click.option('--original-size/--no-original-size', default=True,
              help='Don\'t keep width and height equals to PSD.')
def __render_layer(file, output, layer, original_size):
    """Compose a PSD Layer to PNG."""
    image = psdtemplater.render_layer(file, layer, original_size=original_size)
    file_path = Path(file)
    output_path = Path(output)
    if output_path.is_dir():
        filaname = (file_path.stem + '.' +
...
__cli.add_command(__render_layer)
